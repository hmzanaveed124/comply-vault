...
export default function CapacityLab({reading}:{reading:Reading[]}) {
 ...
 {multi&&<><Field label="Firms supervised" value={inputs.firms} max={1000} onChange={v=>set('firms',Math.max(1,v))}/><label className={styles.field}><span>Workload numbers below cover</span><select value={inputs.scope} onChange={e=>set('scope',e.target.value)}><option value="portfolio">My entire portfolio</option><option value="firm">One average firm</option></select><small>{inputs.scope==='firm'?'Workload is multiplied by '+result.firms+' firms.':'Workload is counted once across all firms.'} Prices, setup and overhead always cover the entire portfolio.</small></label></>}
 ...
 <section className={styles.panel}><div className={styles.panelTop}><h2>Where capacity comes back</h2><span className={styles.pill}>{number(result.effectiveReduction)}% effective reduction</span></div><WorkflowChart result={result}/><details><summary>See exact annual hours</summary><div className={styles.tableWrap}><table><thead><tr><th>Workflow</th><th>Current hours</th><th>Hours released*</th></tr></thead><tbody>{result.rows.map(r=><tr key={r.key}><td>{r.label}</td><td>{r.annual.toFixed(1)}</td><td>{r.saved.toFixed(1)}</td></tr>)}</tbody></table></div><p className={styles.small}>*At full adoption, before extra oversight. The financial model applies the first-year ramp separately.</p></details></section>
 ...
 {multi&&<p className={styles.insight}>{result.capacity===null?'Enter the annual workload for one additional firm to model engagement capacity.':result.capacity.toFixed(1)+' firm-equivalents of annual capacity at full adoption.'} This is a workload comparison, not a forecast of additional clients, revenue or staffing capacity.</p>}
 </section>
